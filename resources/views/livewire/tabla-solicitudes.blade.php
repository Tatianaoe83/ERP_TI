<div x-data="solicitudesData()">
                ...
                <div x-show="!cargando && solicitudSeleccionada" class="space-y-6">
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3 p-4 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                            <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Nombre</p><p class="text-sm font-medium text-slate-900 dark:text-slate-100" x-text="solicitudSeleccionada?.empleado?.NombreEmpleado"></p></div>
                            <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Correo</p><p class="text-sm text-slate-900 dark:text-slate-100" x-text="solicitudSeleccionada?.empleado?.Correo"></p></div>
                            <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Gerencia</p><p class="text-sm text-slate-900 dark:text-slate-100" x-text="solicitudSeleccionada?.gerencia?.NombreGerencia || 'N/A'"></p></div>
                            <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Obra</p><p class="text-sm text-slate-900 dark:text-slate-100" x-text="solicitudSeleccionada?.obra?.NombreObra || 'N/A'"></p></div>
                            <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Puesto</p><p class="text-sm text-slate-900 dark:text-slate-100" x-text="solicitudSeleccionada?.puesto?.NombrePuesto || 'N/A'"></p></div>
                            <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Ubicación</p><p class="text-sm text-slate-900 dark:text-slate-100" x-text="solicitudSeleccionada?.ProyectoNombre || solicitudSeleccionada?.Proyecto || 'N/A'"></p></div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="p-4 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-3">
                            <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Motivo</p><p class="text-sm font-medium text-slate-900 dark:text-slate-100" x-text="solicitudSeleccionada?.Motivo || 'N/A'"></p></div>
                            <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Descripción</p><p class="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap" x-text="solicitudSeleccionada?.DescripcionMotivo || 'N/A'"></p></div>
                            <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Requerimientos</p><p class="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap" x-text="solicitudSeleccionada?.Requerimientos || 'N/A'"></p></div>
                            <div class="grid grid-cols-2 gap-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                                ...
                                <div><p class="text-xs text-slate-500 dark:text-slate-400 mb-0.5">Creado</p><p class="text-sm text-slate-900 dark:text-slate-100" x-text="solicitudSeleccionada?.fechaCreacion || 'N/A'"></p></div>
                            </div>
                                        ...
                                        <div x-show="getIdx(grupo.numeroPropuesta) === ci" class="p-4">
                                            <div class="rounded-lg border p-4" :class="cot.esGanador ? 'border-emerald-300 dark:border-emerald-700 bg-emerald-50/30 dark:bg-emerald-900/10' : 'border-rose-200 dark:border-rose-800 bg-rose-50/20 dark:bg-rose-900/10'">
                                                ...
                                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
                                                    <div><p class="text-xs text-slate-500 mb-0.5">Proveedor</p><p class="font-medium text-slate-900 dark:text-slate-100" x-text="cot.Proveedor || 'N/A'"></p></div>
                                                    <div><p class="text-xs text-slate-500 mb-0.5">No. Parte</p><p class="font-mono text-slate-900 dark:text-slate-100" x-text="cot.NumeroParte || 'N/A'"></p></div>
                                                    <div><p class="text-xs text-slate-500 mb-0.5">Precio Unit.</p><p class="font-semibold text-slate-900 dark:text-slate-100" x-text="cot.Precio != null ? ('$' + parseFloat(cot.Precio).toLocaleString('es-MX', {minimumFractionDigits:2})) : 'N/A'"></p></div>
                                                    <div><p class="text-xs text-slate-500 mb-0.5">Envío</p><p class="text-slate-900 dark:text-slate-100" x-text="'$' + parseFloat(cot.CostoEnvio||0).toLocaleString('es-MX', {minimumFractionDigits:2})"></p></div>
                                                    <div><p class="text-xs text-slate-500 mb-0.5">Total</p><p class="font-bold" :class="cot.esGanador ? 'text-emerald-700 dark:text-emerald-300' : 'text-slate-900 dark:text-slate-100'" x-text="'$' + (parseFloat(cot.Precio||0)+parseFloat(cot.CostoEnvio||0)).toLocaleString('es-MX', {minimumFractionDigits:2})"></p></div>
                                                </div>
                                            </div>
                                        </div>
                                    </template>
    ...
    <div class="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/70 backdrop-blur-sm"
        ...
        <div class="relative z-10 w-full max-w-7xl mx-2 lg:mx-6 bg-gray-50 dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh] overflow-hidden"
            ...
            <div class="px-6 py-2.5 bg-amber-50 dark:bg-amber-950/30 border-b border-amber-200 dark:border-amber-800/40 shrink-0">
                <p class="text-xs text-amber-700 dark:text-amber-300">
                    <i class="fas fa-exclamation-triangle text-amber-500 mr-1"></i>
                    XML pendiente para: <span class="font-bold">{{ implode(', ', $proveedoresSinFactura) }}</span>.
                    Para proveedores extranjeros (Starlink, AWS) esta sección no aplica.
                </p>
            </div>
            @endif
            @endif
                ...
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900"
                    ...
                    <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
                        <div>
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ $p['proveedor'] ?? 'Sin proveedor' }} &middot; {{ (int)($p['itemsTotal'] ?? 0) }} unidades</p>
                        </div>
                        <div class="text-right">
                            <p class="text-xs text-slate-500 mb-0.5">Precio unitario</p>
                    ...
                    <div class="hidden lg:grid grid-cols-12 gap-4 px-5 py-2 bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                        ...
                        <div class="col-span-2">XML Factura</div>
                    ...
                    <div class="px-5 py-4" wire:key="unit-{{ $asignacionSolicitudId }}-{{ $pIndex }}-{{ $uIndex }}">
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
                            <div class="col-span-1 flex lg:justify-center">
                                <span class="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-bold">
                                    {{ $u['unidadIndex'] ?? ($uIndex + 1) }}
                                </span>
                            </div>
                            <div class="col-span-3">
                                <p class="lg:hidden text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Descripción</p>
                                <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ $p['nombreEquipo'] ?? 'Producto' }}</p>
                                <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Unidad {{ $u['unidadIndex'] ?? ($uIndex + 1) }}</p>
                            </div>
                            <div class="col-span-2">
                                <p class="lg:hidden text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">XML Factura</p>
                                @php
                                        ...
                                        <div class="flex flex-col gap-1">
                                            <label class="inline-flex items-center gap-2 h-9 px-3 rounded-lg border-2 border-dashed cursor-pointer transition-all text-xs
                                                ...
                                                <span class="font-medium truncate max-w-[7rem]">
                                                    @if($hasNewXml)
                                                        {{ $parsedOk ? 'XML válido (' . count($parsed['conceptos']) . ')' : 'XML cargado' }}
                    ...
                    foreach ($propuestasAsignacion as $pi => $p) {
                        foreach (($p['unidades'] ?? []) as $ui => $u) {
                            ...
                            if ($parsed && empty($parsed['error']) && (!empty($parsed['conceptos']) || !empty($parsed['total']))) {
                                ...
                                $todasFacturasParseadas->push(['uuid' => $uuid, 'emisor' => $parsed['emisor'] ?? '', 'mes' => $parsed['mes'] ?? '', 'anio' => $parsed['anio'] ?? '', 'total' => $parsed['total'] ?? '0', 'moneda' => $parsed['moneda'] ?? 'MXN', 'conceptos' => $parsed['conceptos'] ?? [], 'pIndex' => $pi, 'uIndex' => $ui]);
                            }
                        }
                    }
                    $totalConceptos = $todasFacturasParseadas->sum(fn($f) => count($f['conceptos']));
                    $conceptosSinMatch = $todasFacturasParseadas->sum(fn($f) => collect($f['conceptos'])->filter(fn($c) => empty($c['insumoId']))->count());
                    $conceptosConMatch = $totalConceptos - $conceptosSinMatch;
                @endphp
                ...
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden" x-data="{ expandido: true }">
                    <button type="button" @click="expandido = !expandido"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="text-left">
                                <p class="text-sm font-semibold text-slate-800 dark:text-slate-200">Conceptos del XML</p>
                                <p class="text-xs text-slate-500">{{ $totalConceptos }} concepto(s) &middot; {{ $todasFacturasParseadas->count() }} factura(s)</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            @if($conceptosSinMatch > 0)
                            <span class="text-[11px] font-bold px-2 py-1 rounded bg-amber-100 text-amber-700 border border-amber-200 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-700/50">{{ $conceptosSinMatch }} sin insumo</span>
                            @endif
                            @if($conceptosConMatch > 0)
                            <span class="text-[11px] font-bold px-2 py-1 rounded bg-emerald-100 text-emerald-700 border border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-700/50">{{ $conceptosConMatch }} asignados</span>
                            @endif
                    ...
                    <div x-show="expandido" x-transition>
                        <div class="px-5 py-2.5 bg-sky-50 dark:bg-slate-800 border-b border-sky-100 dark:border-slate-700">
                            <p class="text-xs text-sky-700 dark:text-sky-400">
                                ...
                                Revisa cada concepto y asigna el insumo correcto del catálogo.
                            </p>
                        </div>

                        @foreach($todasFacturasParseadas as $fIdx => $facturaData)  
                        @php $fPIndex = $facturaData['pIndex']; $fUIndex = $facturaData['uIndex']; @endphp
                        <div class="p-5 {{ !$loop->last ? 'border-b border-slate-100 dark:border-slate-800' : '' }}">
                            <div class="flex flex-wrap items-center gap-3 mb-4 text-xs">
                                @if($facturaData['emisor'])
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-medium">
                                    <i class="fas fa-building text-slate-400"></i>{{ $facturaData['emisor'] }}
                                </span>
                                @endif
                                @if($facturaData['uuid'])
                                <span class="font-mono text-slate-400 dark:text-slate-500">{{ Str::upper(Str::limit($facturaData['uuid'], 20)) }}</span>
                                @endif
                                @if($facturaData['total'])
                                <span class="font-bold text-emerald-700 dark:text-emerald-300">${{ number_format((float)$facturaData['total'], 2) }} {{ $facturaData['moneda'] }}</span>
                                @endif
                            </div>

                            <div class="rounded-lg border border-slate-200 dark:border-slate-700 overflow-visible">
                                <table class="w-full text-xs">
                                    <thead class="bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400">
                                        <tr>
                                            <th class="text-left px-4 py-2.5 font-medium">Descripción</th>
                                            <th class="text-right px-3 py-2.5 font-medium w-14">Cant.</th>
                                            <th class="text-right px-3 py-2.5 font-medium w-24">Unitario</th>
                                            <th class="text-right px-3 py-2.5 font-medium w-24">Importe</th>
                                            <th class="text-left px-3 py-2.5 font-medium w-52">Insumo</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800 bg-gray-50 dark:bg-slate-900">
                                        @foreach($facturaData['conceptos'] as $cIdx => $concepto)
                                        @php
                                            $tieneMatch = !empty($concepto['insumoId']);
                                            $insumoActual = $tieneMatch ? (int)$concepto['insumoId'] : null;
                                            $nombreInsumoActual = '';
                                            if ($insumoActual) {
                                        ...
                                        @endphp
                                        <tr class="{{ $tieneMatch ? '' : 'bg-amber-50/30 dark:bg-amber-950/10' }} hover:bg-slate-50 dark:hover:bg-slate-800/50"
                                            wire:key="concepto-{{ $fPIndex }}-{{ $fUIndex }}-{{ $cIdx }}">
                                            <td class="px-4 py-3">
                                                <div class="flex items-start gap-2">
                                                    <div class="mt-0.5 w-4 h-4 rounded flex items-center justify-center shrink-0 {{ $tieneMatch ? 'bg-emerald-100 dark:bg-emerald-900/40' : 'bg-amber-100 dark:bg-amber-900/30' }}">
                                                        <i class="fas {{ $tieneMatch ? 'fa-check text-emerald-600' : 'fa-exclamation text-amber-600' }}" style="font-size:7px"></i>
                                                    </div>
                                                    <span class="text-slate-800 dark:text-slate-200">{{ Str::limit($concepto['nombre'], 60) }}</span>
                                                </div>
                                            </td>
                                            <td class="px-3 py-3 text-right text-slate-600 dark:text-slate-300 font-medium">{{ $concepto['cantidad'] }}</td>
                                            <td class="px-3 py-3 text-right text-slate-600 dark:text-slate-300 font-mono">${{ number_format((float)($concepto['costo'] ?? 0), 2) }}</td>
                                            <td class="px-3 py-3 text-right font-bold text-slate-800 dark:text-slate-200 font-mono">${{ number_format((float)($concepto['importe'] ?? 0), 2) }}</td>

                                            <td class="px-3 py-3"
                                                x-data="{
                                                    abierto: false,
                                                    busqueda: '',
                                                    rect: { top: 0, left: 0, width: 0, bottom: 0 },
                                                    wireId: null,
                                                    get filtrados() {
                                                        const q = this.busqueda.trim().toLowerCase();
                                                        if (!q) return [];
                                                        const todos = window.__insumosCatalogo || [];
                                                        return todos.filter(i => i.nombre.toLowerCase().includes(q)).slice(0, 40);
                                                    },
                                                    abrirPanel(event) {
                                                        ...
                                                        this.busqueda = '';
                                                        this.$nextTick(() => {
                                                            const inp = document.getElementById('si-{{ $fPIndex }}-{{ $fUIndex }}-{{ $cIdx }}');
                                                            if (inp) inp.focus();
                                                        });
                                                    },
                                                    cerrarPanel() { this.abierto = false; },
                                                    onScroll() { if (this.abierto) this.cerrarPanel(); }
                                                }"
                                                x-init="
                                                    wireId = $el.closest('[wire\\:id]').getAttribute('wire:id');
                                                    window.addEventListener('scroll', onScroll.bind($data), true);
                                                    $cleanup(() => window.removeEventListener('scroll', onScroll.bind($data), true));
                                                "
                                                @click.outside="cerrarPanel()">
                                                    <button type="button"
                                                        ...
                                                        class="w-full flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border transition-colors text-left group
                                                            {{ $tieneMatch
                                                                ? 'bg-emerald-50 border-emerald-200 hover:bg-emerald-100 active:bg-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-700/50 dark:hover:bg-emerald-900/40 dark:active:bg-emerald-900/60'
                                                                : 'bg-amber-50 border-dashed border-2 border-amber-300 hover:bg-amber-100 hover:border-amber-400 dark:bg-amber-900/30 dark:border-amber-700/50 dark:hover:bg-amber-900/50' }}">
                                                        ...
                                                        @if($tieneMatch)
                                                            ...
                                                            <i class="fas fa-pencil-alt text-[8px] text-emerald-400 dark:text-emerald-500 shrink-0 opacity-40 group-hover:opacity-100 transition-opacity"></i>
                                                        @else
                                                ...
                                                <template x-teleport="body">
                                                    <div x-show="abierto"
                                                        ...
                                                        :style="`
                                                            ...
                                                            width: ${Math.max(rect.width, 248)}px;
                                                        ...
                                                        class="rounded-xl border border-slate-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 shadow-2xl ring-1 ring-black/5 dark:ring-white/5 overflow-hidden">

                                                        <div class="p-2.5 bg-gray-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700">
                                                            <div class="relative">
                                                                <input
                                                                    type="text"
                                                                    id="si-{{ $fPIndex }}-{{ $fUIndex }}-{{ $cIdx }}"
                                                                    x-model.debounce.150ms="busqueda"
                                                                    placeholder="Escribe para buscar insumo..."
                                                                    @keydown.escape="cerrarPanel()"
                                                                    class="w-full h-8 pl-8 pr-3 text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-800 text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-400/20 transition-all">
                                                                <i class="fas fa-search text-[9px] text-slate-400 dark:text-slate-500 absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none"></i>
                                                            </div>
                                                        </div>

                                                        <div class="max-h-52 overflow-y-auto overscroll-contain bg-gray-50 dark:bg-slate-900">
                                                            @if($tieneMatch)
                                                            <button type="button"
                                                                @click="Livewire.find(wireId).actualizarInsumoConcepto({{ $fPIndex }}, {{ $fUIndex }}, {{ $cIdx }}, null); cerrarPanel()"
                                                                ...
                                                                <span>Quitar insumo asignado</span>
                                                            </button>
                                                            @endif
                                                            ...
                                                            <div x-show="busqueda.trim() === ''"
                                                                ...
                                                                <span class="text-xs text-slate-400 dark:text-slate-500">Escribe al menos una letra para buscar</span>
                                                            </div>

                                                            <template x-if="busqueda.trim() !== '' && filtrados.length === 0">
                                                                <div class="px-4 py-6 text-center flex flex-col items-center gap-2">
                                                                    ...
                                                                    <span class="text-xs text-slate-400 dark:text-slate-500">Sin resultados para "<span x-text="busqueda" class="font-medium text-slate-500 dark:text-slate-400"></span>"</span>
                                                                </div>
                                                            </template>

                                                            <template x-for="insumo in filtrados" :key="insumo.id">
                                                                <button type="button"
                                                                    @click="Livewire.find(wireId).actualizarInsumoConcepto({{ $fPIndex }}, {{ $fUIndex }}, {{ $cIdx }}, insumo.id); cerrarPanel()"
                                                                    ...
                                                                    <span x-text="insumo.nombre" class="truncate"></span>
                                                                </button>
                                                            </template>
                                                        </div>
                                                        ...
                                                        <div class="px-3 py-2 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 flex items-center justify-between">
                                                            <span class="text-[10px] text-slate-400 dark:text-slate-500"
                                                                x-text="busqueda.trim() ? filtrados.length + ' resultado(s)' : (window.__insumosCatalogo || []).length + ' insumos disponibles'">
                                                            </span>
                                                            <button type="button" @click="cerrarPanel()"
                                                                class="text-[10px] text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-200 transition-colors px-2 py-0.5 rounded hover:bg-slate-200 dark:hover:bg-slate-700">
            ...
            <div class="px-6 py-4 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between gap-3 bg-gray-50 dark:bg-slate-900 shrink-0">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="button"
                        ...
                        "
                        class="px-4 py-2 text-sm rounded-lg font-medium text-white bg-slate-900 hover:bg-slate-800 dark:bg-slate-700 dark:hover:bg-slate-600 transition-colors disabled:opacity-50">
...
document.addEventListener('livewire:load', function () {
    const _swalBase = { background: '#f9fafb', color: '#1e293b', confirmButtonColor: '#3b82f6', cancelButtonColor: '#94a3b8' };
    ...
    window.addEventListener('swal:success', function (e) {
        ...
        Swal.fire({ ..._swalBase, icon: 'success', title: 'Listo', text: msg, timer: 3500, showConfirmButton: false, toast: true, position: 'top-end' });
    });
    ...
    window.addEventListener('swal:error', function (e) {
        ...
        Swal.fire({ ..._swalBase, icon: 'error', title: 'Error', text: msg });
    });
    ...
    window.addEventListener('swal:warning', function (e) {
        ...
        Swal.fire({ ..._swalBase, icon: 'warning', title: 'Atencion', text: msg });
    });
    ...
    window.addEventListener('swal:info', function (e) {
        ...
        if (!msg) return;
        Swal.fire({ ..._swalBase, icon: 'info', text: msg, timer: 3000, showConfirmButton: false, toast: true, position: 'top-end' });
    });
});
</script>