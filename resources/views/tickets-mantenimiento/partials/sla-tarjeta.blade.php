@if(!empty($sla))
<div class="kpi-chip kpi-chip--{{ $sla['estado'] ?? ($sla['estado_sla'] ?? 'en_tiempo') }}">
    <div class="flex items-center justify-between gap-2 mb-1.5">
        <span class="text-[10px] font-semibold uppercase tracking-wide opacity-80">SLA</span>
    ...
    <div class="h-1.5 rounded-full bg-black/10 dark:bg-white/15 overflow-hidden mb-1.5">
        <div class="h-full rounded-full transition-all {{ $sla['estilo']['bar'] ?? 'bg-current' }}"
    ...
    <p class="mt-1 text-[11px] font-semibold flex items-center gap-1">
