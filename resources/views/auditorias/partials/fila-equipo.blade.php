...
<tr data-fila="{{ $fila->id }}" data-marca="{{ $marca }}"
    ...
    <td>
        <label class="aud-sr" for="obs-{{ $fila->id }}">Observaciones de {{ $etiqueta }}</label>
        <textarea id="obs-{{ $fila->id }}" class="aud-obs" rows="1" maxlength="2000"
                  placeholder="{{ $esBaja ? 'Baja de inventario…' : 'Anotar hallazgo…' }}"
                  data-fila="{{ $fila->id }}" data-campo="observaciones"
