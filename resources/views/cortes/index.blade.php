...
<script>
    $(function() {
        ...
        const table = $('#tabla').DataTable({
            ...
            language: {
                zeroRecords: "<div class='py-8 text-center text-slate-400 dark:text-slate-500 italic'>No se encontraron resultados</div>",
                ...
                paginate: {
                    first: '<<', last: '>>', next: '<i class="fas fa-chevron-right text-[10px]"></i>', previous: '<i class="fas fa-chevron-left text-[10px]"></i>'
                }
            },
            ajax: {
        ...
        // ==========================================
        //  SOLUCIÓN A LOS BUGS DE REFRESH VISUAL
        // ==========================================

        // 1. SELECT (Monto Base): Actualización manual sin draw()
        $('#tabla tbody').on('change', '.monto-select', function() {
            ...
            // Actualizamos data en memoria (para el botón Guardar)
            data.SelectedIndex = idx;
            
            // Calculo visual directo
        ...
        // 2. INPUT (Margen): Actualización manual sin cell().data()
        ...
        $('#guardar').on('click', async function() {
            ...
            if (!gid) {
                Swal.fire({ icon: 'error', title: 'Falta gerencia', confirmButtonColor: '#4f46e5' });
                return;
            }
            ...
            // Iteramos sobre las filas del DOM para obtener los valores más frescos
            $('#tabla tbody tr').each(function(i) {
                const row = table.row(this).data();
                if(!row) return;

                // Capturamos el valor visual actual
                ...
                for (const mp of montos) {
                    if (Number(mp.Costo) > 0) {
                        ...
                        payload.push({
                            NombreInsumo: String(row.NombreInsumo || 'SIN_NOMBRE'),
                            Mes: mp.Mes, Costo: +Number(mp.Costo).toFixed(2), Margen: +margenVal.toFixed(2),
                            CostoTotal: costoTotal, GerenciaID: Number(gid)
                        });
                    }
                }
            });
            ...
            try {
                ...
                Swal.fire({ icon: 'success', title: 'Guardado', timer: 1500, showConfirmButton: false }).then(() => location.reload());
            } catch (e) {
