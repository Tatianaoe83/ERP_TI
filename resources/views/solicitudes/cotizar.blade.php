...
<div
    ...
    <!-- Header -->
    <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-4 mb-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex items-center gap-3 flex-wrap">
                <a href="{{ route('tickets.index') }}"
                    class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-sm font-medium no-underline">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-5 mb-4">
                                            ...
                                            <td class="px-3 py-2">
                                                <div class="flex items-center justify-end gap-1">
                                                    ...
                                                    <input type="number" step="0.01" min="0" x-model.number="cot.precioUnitario" placeholder="0"
                                                        ...
                                                        class="w-full px-2 py-1 text-sm text-right border border-slate-300 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
                                                </div>
                                            </td>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('cotizarPagina', (solicitudId, ticketsUrl = '/tickets') => ({
        ...
        async guardar() {
            const { proveedores, productos } = this.buildPayload();
            if (!productos.length) {
            ...
            try {
                const res = await fetch(`/solicitudes/${this.solicitudId}/guardar-cotizaciones`, {
                    ...
                    body: JSON.stringify({ proveedores, productos })
                });
                const data = await res.json().catch(() => ({}));
                Swal.close();
                if (data.success) {
