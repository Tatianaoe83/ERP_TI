...
<body>
    <div class="header">
        <h1>Listado de Equipos Asignados</h1>
    ...
    <div class="filters">
        ...
        @if(!empty($filtros['equipo_id']))
            @php
                $equipo = \App\Models\Equipos::find($filtros['equipo_id']);
            @endphp
            <div class="filter-item">
                <span class="filter-label">Equipo:</span>
                <span class="filter-value">{{ $equipo ? $equipo->Nombre . ' - ' . $equipo->Marca . ' ' . $equipo->Modelo : 'N/A' }}</span>
            </div>
        @endif
    ...
    @if($resultado->count() > 0)
            ...
            <tbody>
                @foreach($resultado as $item)
                    <tr>
                        ...
                        <td>{{ $item->equipo_marca }} {{ $item->equipo_modelo }}</td>
                        <td>{{ $item->equipo_serie }}</td>
        ...
        <div class="summary">
            ...
            @foreach($estatusCounts as $estatus => $count)
                <p><strong>{{ $estatus }}:</strong> {{ $count }} equipos</p>
            @endforeach
        </div>
    @else
        <div style="text-align: center; padding: 40px; color: #6b7280;">
            <h3>No se encontraron resultados</h3>
            <p>No hay equipos asignados que coincidan con los filtros aplicados.</p>
        </div>
    @endif
