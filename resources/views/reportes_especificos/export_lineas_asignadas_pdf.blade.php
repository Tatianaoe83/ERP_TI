...
<body>
    <div class="header">
        <h1>Listado de Líneas Asignadas</h1>
    ...
    <div class="filters">
        ...
        @if(!empty($filtros['linea_id']))
            @php
                $linea = \App\Models\LineasTelefonicas::find($filtros['linea_id']);
            @endphp
            <div class="filter-item">
    ...
    @if($resultado->count() > 0)
        ...
        <div class="summary">
            ...
            @foreach($estatusCounts as $estatus => $count)
                <p><strong>{{ $estatus }}:</strong> {{ $count }} líneas</p>
            @endforeach
        </div>
    @else
        <div style="text-align: center; padding: 40px; color: #6b7280;">
            <h3>No se encontraron resultados</h3>
            <p>No hay líneas asignadas que coincidan con los filtros aplicados.</p>
        </div>
    @endif
