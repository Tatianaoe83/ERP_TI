...
<style>
    ...
    .zona-archivo-xml.tiene-archivo {
        border-color: #818cf8;
        background-color: rgba(238,242,255,0.6);
    }
    .dark .zona-archivo-xml.tiene-archivo {
        background-color: rgba(49,46,129,0.2);
    }
    .zona-archivo-pdf.tiene-archivo {
        border-color: #34d399;
        background-color: rgba(236,253,245,0.6);
    }
    .dark .zona-archivo-pdf.tiene-archivo {
            ...
            <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5 bg-gray-50 dark:bg-slate-900">
                ...
                <div id="seccionConceptos" class="hidden rounded-xl border border-violet-200 dark:border-violet-800/40 overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-xs">
                            ...
                            <tbody id="tbodyConceptos" class="divide-y divide-violet-100 dark:divide-violet-900/30">
                                {{-- JS la llena --}}
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <p id="errFdNombre" class="hidden mt-1.5 text-xs text-red-500 flex items-center gap-1"><i class="fas fa-exclamation-circle"></i> Campo requerido</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p id="errFdGerencia" class="hidden mt-1.5 text-xs text-red-500 flex items-center gap-1"><i class="fas fa-exclamation-circle"></i> Seleccione una gerencia</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p id="errFdInsumo" class="hidden mt-1.5 text-xs text-red-500 flex items-center gap-1"><i class="fas fa-exclamation-circle"></i> Seleccione un insumo</p>
                    </div>

                    <div class="hidden">
                        <input type="text" id="fdInsumoNombre">
                    </div>
                    ...
                    <div>
                        ...
                        <p id="errFdCosto" class="hidden mt-1.5 text-xs text-red-500 flex items-center gap-1"><i class="fas fa-exclamation-circle"></i> Campo requerido</p>
                    </div>
                ...
                <div class="flex items-start gap-3 px-4 py-3 rounded-xl bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                        Esta factura <span class="font-semibold text-slate-600 dark:text-slate-300">no estará asociada a ninguna solicitud</span> — el campo SolicitudID quedará en <code class="px-1.5 py-0.5 rounded-md bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-[11px] font-mono">N/A</code>.
                    </p>
                </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// MODAL FACTURA DIRECTA — con lectura XML + PDF (idéntico a Solicitudes)
// ─────────────────────────────────────────────────────────────────────────────
(function () {
    /*
     * state.origenParsed: 'xml' | 'pdf' | null
     *   Indica qué archivo fue el último en poblar los campos parsados.
     *   El XML siempre tiene prioridad: si ya hay XML parseado, un PDF
     *   NO sobreescribirá los campos, solo mostrará la previsualización.
     */
    const state = {
        xmlFile:      null,
        pdfFile:      null,
        parsed:       null,    // datos del parse activo
        origenParsed: null,    // 'xml' | 'pdf'
    };
    ...
    function resetModal() {
        ...
        ['fdNombre','fdGerencia','fdInsumoSelect','fdInsumoNombre',
         'fdImporte','fdCosto','fdMes','fdAnio'].forEach(id => {
            if ($(id)) $(id).value = '';
        });

        hide('seccionParsed'); hide('seccionConceptos');
        hide('spinnerXml');    hide('spinnerPdf');
        hide('errorXml');
        hide('errFdNombre');   hide('errFdCosto');
        hide('errFdGerencia'); hide('errFdInsumo');
        hide('avisoPdfExtranjero'); hide('avisoPdfSobreescribe');
    ...
    function resetZonaXml() {
        ...
        $('iconoXml').innerHTML =
            '<i class="fas fa-file-code text-violet-500 dark:text-violet-400 text-lg transition-colors"></i>';
    }
    ...
    function resetZonaPdf() {
        ...
        $('iconoPdf').innerHTML =
            '<i class="fas fa-file-pdf text-slate-400 dark:text-slate-500 group-hover:text-emerald-500 text-lg transition-colors"></i>';
    }

    // ── POBLAR CAMPOS DESDE DATOS PARSEADOS ───────────────────────────────────
    function poblarCampos(data, origen) {
        /*  origen: 'xml' | 'pdf'
            Si ya había un XML parseado, un PDF NO sobreescribe los campos. */
        const yaHayXml = state.origenParsed === 'xml';
        if (origen === 'pdf' && yaHayXml) {
            // Solo mostrar aviso; no sobreescribir
            show('avisoPdfSobreescribe');
            return;
        }
        ...
        // Llenar campos si están vacíos (no pisar lo que el usuario ya escribió)
        ...
        // Estilo del panel según origen
        const panelBorder = origen === 'xml'
            ? 'border-indigo-200 dark:border-indigo-800/40'
            : 'border-emerald-200 dark:border-emerald-800/40';
        const panelBg = origen === 'xml'
            ? 'bg-indigo-50/60 dark:bg-indigo-950/20'
            : 'bg-emerald-50/50 dark:bg-emerald-950/20';
        const headerBg = origen === 'xml'
            ? 'bg-indigo-100/80 dark:bg-indigo-900/30 border-indigo-200 dark:border-indigo-800/40'
            : 'bg-emerald-100/60 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800/40';
        const iconClass = origen === 'xml'
            ? 'fas fa-check-circle text-indigo-500 dark:text-indigo-400 text-xs'
            : 'fas fa-file-pdf text-emerald-500 dark:text-emerald-400 text-xs';
        const headerText = origen === 'xml'
            ? 'Datos detectados del XML'
            : 'Datos extraídos del PDF (proveedor extranjero)';
        const textColor = origen === 'xml'
        ...
        $('seccionParsed').className =
            `rounded-xl border overflow-hidden`;

        show('seccionParsed');

        // Tabla de conceptos
        renderConceptos(data.conceptos ?? [], origen);
    }

    function renderConceptos(conceptos, origen) {
        ...
        conceptos.forEach(c => {
            ...
            const tieneMatch = !!c.insumoId;
            const badgeMatch = tieneMatch
                ? `<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-700">
            ...
            tr.innerHTML = `
                ...
                <td class="px-3 py-2.5 text-center">${badgeMatch}</td>
            `;
    ...
    async function parsearXml(file) {
        ...
        try {
            ...
            $('textoXml').textContent = file.name.length > 22
                ? file.name.substring(0, 22) + '…'
                : file.name;
            $('iconoXml').innerHTML =
                '<i class="fas fa-check-circle text-violet-500 text-lg"></i>';

            // Ocultar aviso de "sobreescribe" si se subió XML después del PDF
            hide('avisoPdfSobreescribe');
    ...
    // ── PARSEAR PDF (proveedor extranjero) ────────────────────────────────────
    async function parsearPdf(file) {
        ...
        try {
            ...
            if (res.ok && !data.error) {
                // Si no hay XML previo: poblar campos
                poblarCampos(data, 'pdf');
                if (state.origenParsed !== 'xml') {
                    show('avisoPdfExtranjero');
                }
            }
            // Si hay XML previo poblarCampos() ya habrá mostrado 'avisoPdfSobreescribe'

        } catch (_) {
            // Silencioso: el PDF puede no tener datos extraíbles
        } finally {
            hide('spinnerPdf');
        }
    }
    ...
    function validar() {
        ...
        if (!$('fdCosto').value || parseFloat($('fdCosto').value) < 0) {
            show('errFdCosto'); ok = false;
        }
        return ok;
    }
    ...
    async function guardar() {
        ...
        try {
            ...
            if (window.Swal) {
                Swal.fire({
                    icon: 'success', title: '¡Factura guardada!',
    ...
    function setupDragDrop(zonaId, inputId, accept, onFile) {
        const zona  = $(zonaId);
        const input = $(inputId);
        ...
        ['dragleave','dragend'].forEach(ev => {
            zona.addEventListener(ev, () => zona.classList.remove('dragging'));
        });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $('inputXml').addEventListener('change', async function () {
            await handleXmlFile(this.files[0]);
        });

        setupDragDrop('zonaXml', 'inputXml', '.xml', async file => {
            $('inputXml').value = '';   // limpiar para que change dispare de nuevo si es necesario
            state.xmlFile = file;
            await parsearXml(file);
        });
        ...
        async function handlePdfFile(file) {
            ...
            // Nombre en zona
            $('textoPdf').textContent = file.name.length > 22
                ? file.name.substring(0, 22) + '…'
                : file.name;
            $('iconoPdf').innerHTML =
            ...
            // Extraer datos del PDF (idéntico a Solicitudes)
            await parsearPdf(file);
        }

        $('inputPdf').addEventListener('change', async function () {
            await handlePdfFile(this.files[0]);
        });

        setupDragDrop('zonaPdf', 'inputPdf', '.pdf', async file => {
